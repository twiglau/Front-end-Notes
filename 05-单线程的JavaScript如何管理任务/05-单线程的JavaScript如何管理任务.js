 ...
 * 
 * 上述过程会不断重复,这就是JavaScript的运行机制,称为事件循环机制(Event Loop).
 * 
 * Event Loop的设计会带俩一些问题,比如setTimeout,setInterval的事件精确性. 这两个方法会设置一个计时器,当计时器
 * 完成,需要执行回调函数,此时才把回调函数放入回调队列中.
 * 
 * 如果当回调函数放入队列是,假设队列中还有大量的回调函数在等待执行,此时就会造成任务执行时间不精确.
 * 
 * 要优化这个问题,可以使用系统时钟来补偿计时器的不准确性,从而提升精确度. 举个例子,如果你的计时器会在回调时
 * 触发二次计时,可以在每次回调任务结束的时候,根据最初的系统时间和该任务的执行时间进行插值比较,来修正后续的计时器时间.
 */

/**
 * 五, Node.js 中的 Event Loop.
 * 除了浏览器, Node.js中同样存在Event Loop,由于 JavaScript是单线程的, Event Loop的设计使 Node.js
 * 可以通过将操作转移到系统内核中,来执行非阻塞I/O操作.
 * 
 * Node.js中的事件循环执行过程为:
 * > 1. 当Node.js启动时将初始化事件循环,处理提供的输入脚本;
 * > 2. 提供的输入脚本可以进行异步API调用,然后开始处理时间循环;
 * > 3. 在事件循环的每次运行之间,Node.js会检查它是否正在等待任何异步I/O或计时器,如果没有,则将其干净第关闭.
 * 
 * 有浏览器不一样,Node.js中时间循环分成不同的阶段:
 * 
   ┌───────────────────────────┐
 ...
 * 如图 05.3 示例图
 * 
 * 由于事件循环阶段划分不一致,Node.js和浏览器在对宏任务和微任务的处理上也不一样.
 */
